...
class Trainer():
    def __init__(self, config:Dict[str, Any]):
        ...
        self.field_mapping = [
            ...
            Mapping('lr', 'Learning Rate', '', length=22),
    ...
    def eval(self, ds: DataLoader, i: int) -> None:
        ...
        data = iter(ds)
        local_metric = load_metric(self.metric)
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data, "Evaluating: "):
                ...
                res = model.generate(input_ids, att_masks).cpu()
                labels = batch["labels"].numpy()
                labels = np.where(labels != -100, labels, self.tokenizer.pad_token_id)
                local_metric.add_batch(predictions=self.tokenizer.batch_decode(res), labels=labels.tokenizer.batch_decode(labels))                
                summaries.extend(self.tokenizer.batch_decode(res))
                abstracts.extend(self.tokenizer.batch_decode(labels))
                
            self.writer.add_text('Summary[0]', summaries[0], i)
            metric = self.metric.compute()
            results = rouge.compute(predictions=summaries, labels=abstracts)
            print(results)
            self.logger.add_value('r1',metric['rouge1'].mid.fmeasure)
            self.logger.add_value('r2', metric['rouge2'].mid.fmeasure)
            self.logger.add_value('rl', metric['rougeL'].mid.fmeasure)
            self.logger.print_line(self.eval_mapping)
            
            if self.best_r1 < metric['rouge1'].mid.fmeasure:
                self.best_r1 = metric['rouge1'].mid.fmeasure
                self.save()
                self.evals_since_best = 0
            else:
               self.evals_since_best += 0 
    ...
    def train(self):
        ...
        if self.local_rank not in [-1, 0]:
            dist.barrier()
            ds = get_dataset(self.dataset_name, self.checkpoint, self.max_length, False, self.dataset_prefix)
        else:
            ds = get_dataset(self.dataset_name, self.checkpoint, self.max_length, self.retokenize, self.dataset_prefix)
        ...
        self.lr_scheduler = get_scheduler(
            ...
            num_training_steps=len(dl_train)*self.epochs,
        )
        i = 0
        ...
        epoch = 0
        while True:
            print("Initiating epoch: {}".format(epoch+1))
            if self.distributed:
            ...
            while True:
                ...
                self.eval(dl_eval, i*self.batch_size)

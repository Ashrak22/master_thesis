...
# Adapted from https://github.com/potsawee/longsum0 used by LoBART
# updated for Transformers v4.11 and removed duplication of LongformerSelfAttention
# because in this version the use of gloabl attention is managed by parameter, so that 
# the glbal attention parts don't need to be removed
class LocalSelfAttention(nn.Module):
    def __init__(self, config: BartConfig, layer_id: int, attention_window):
        ...
        localattn_config.attention_window = [0]*(layer_id+1)
        
        localattn_config.attention_window[layer_id] = attention_window

        # self_attn
        self.self_attn = LongformerSelfAttention(localattn_config, layer_id)
    ...
    def forward(
        ...
        # attention_mask = self.attention_mask_mapping(attention_mask)
        self_attn_outputs = self.self_attn.forward(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            #layer_head_mask=layer_head_mask,
            is_index_masked=(attention_mask!=0).transpose(1,2)
        )

        outputs = self.out_proj(self_attn_outputs[0])

        if output_attentions:
            return outputs, self_attn_outputs[1], past_key_value
        else:
            return outputs, None, past_key_value
    ...
    def attention_mask_mapping(self, mask, global_attn_mask=None):
        ...
        new_mask = torch.masked_fill(new_mask, mask==1, -10000)
        return new_mask
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        #attn_probs = torch.masked_fill(attn_probs, is_index_masked[:, :, :, :], 0.0)
